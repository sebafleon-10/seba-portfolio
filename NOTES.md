...
Last updated: September 19, 2026 (orb reveal converge became the globe)
...
## Git Restore Points
- Sep 19 globe session: the orb reveal converge is a turning globe of slots instead of gravity to one point; the canvas owns the converge target, use-orb-reveal no longer writes gravityTarget, prefers-reduced-motion skips the converge and blast. Branch claude/practical-gagarin-ac5790, NOT merged
...
## Current Status
Sep 19 (globe session): Sebastian reported the network was choppy when every particle rushes into the bright orb on section entry. Measured at 1440x900 with a temporary per-frame counter (frame dt, mean speed, mean distance to center), four causes. (1) The boost force t * t * 20 was strongest at the target and applied along a unit vector, so arrived particles vibrated: mean speed stayed 11 to 12 px per frame while distance sat flat at 9px. (2) About 1000 particles in one spatial hash cell pushed frame time from 16.7ms to the 50ms clamp. (3) The hero text repulsion zone (42 percent x, 50 percent y, radius 280) sits 115px from the converge target and is live in every section, so the orb was squeezed into a crescent or an off-center blob. (4) gravityTarget has five writers (who, work, contact, card-stack, useGravityAnchor); when the previous section's anchor scrolled off screen its falling edge set active false about 200ms into the boost, so in Experience and Contact the converge aborted and particles coasted through the center. Sibling: the Experience morph-in (own slot per particle, spring that goes to zero on arrival, staggered departure). Direction A of three picked by Sebastian (A globe, B soft core on the old gravity mechanic, C arrival falloff only; C still dropped to 45ms frames). Now: while gravityBoost is on, the canvas gives every particle a slot on a globe at the viewport center (orbR = min(W, H) * 0.17, latitude fv, longitude fph + t * 0.9 so it turns, the 30 percent with fsel under 0.3 fill the core), a per-particle odelay by distance (up to 220ms plus 50 random), and a clock-based smoothstep ramp over 480ms (dtFrame, so 60 and 120Hz read the same). It reuses the formation's m: the rest spring aims at rest * (1 - m) + slot * m with stiffness 0.04 + 0.05 * m, dots draw with the body recipe (0.62 radius, 3px glow), links under 38px cap 4. free is 0 while boosting, so hero text repulsion and gravity are off; the canvas ignores gravityTarget during the boost and lib/use-orb-reveal.tsx no longer writes it. The old boost constants (force 20, cap 18) are deleted. The blast is untouched. prefers-reduced-motion: no converge, no blast, the label fades in. Measured after: boost frames 16.7ms p95 17.1 in all four sections, identical distance curves in all four (347, 123, 101 at 300, 600, 880ms), and with the boost held open the speed settles to 1 px per frame, which is the rotation alone. Known: in Projects and Contact the globe forms behind the opaque cards, so only its rim and the blast show (the same was true of the old orb). Verified at 1440x900 and 375x812, Experience runs globe, blast, body, pulse in order, no page errors. Headless Chrome is 60Hz; not yet seen on the 120Hz panel. Not merged.

Sep 18 (guide card session): the 002 · EXPERIENCE text card was the quietest object on the stage and said nothing about what to do, so it was rebuilt to hold four things only (Sebastian: the first design was way too busy): company name (mono clamp 32 to 46, compact 22 to 32, text-wrap balance), one sentence (Inter 20/400 white, max 440; the role title is folded into it, the role and dates line is no longer rendered and lives on the role page), a "View the role →" pill (mono 12, 2px border 0.28, 0.6 on hover), and a bottom strip inside the card: "01 / 03" left and a "Next: <company> ↓" button right that scrolls to the next role, or "Next: Projects ↓" on the last role, which scrolls to the section bottom. Goal decided by Sebastian: opening the role and scrolling on are equal. The rail ticks and the arrow are DELETED. The pulse now runs mark to card (formation.pulseDir = -1, the canvas mirrors the front) and lands on the card, not the logo: .exp-hit is added for 260ms at 92 percent of FORMATION_PULSE_MS, snapping the frame to 0.6 white, the name from 0.92 to 1, the pill border to 0.6, and showing a 9px dot on the card's right edge at the name's height (--hit-y), all easing back over 0.7s. No text shadow. The logo kick is gone; the logo keeps its step spring. Card border lives in the .exp-card CSS class (not inline) so the hit class can override it. Mobile gets the same card with no pulse. Verified at 1440x900 and 375x812: Next steps 01 to 02 to 03 to Projects, the pill opens /experience/bts, no page errors. Branch experience-card merged to main (60484f0), live Sep 19.
...
- Text repulsion zone: center x 42%, y 50%, radius 280px, force 3.0 (live in every section; scaled by free, which is 0 during the orb converge)
- Orb converge (Sep 19): while gravityBoost is on the canvas morphs every particle onto a turning globe at the viewport center; see the Sep 19 Current Status entry. The canvas owns that target, never gravityTarget
